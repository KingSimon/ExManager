...
function appContextmenu(ele) {
    ...
    $("#collect").click(function () {
        addCollect(1, $("#modal_app_id").val(), function () {
            $("#appOperation").modal('hide');
            refresh();
        });
    });
    $("#remove-collect").click(function () {
        console.log("#remove-collect");
        removeCollect(1, $("#modal_app_id").val(), function () {
            $("#appOperation").modal('hide');
            refresh();
        });
    });
    ...
    $("#app-disable").click(function () {
        chrome.management.setEnabled($("#modal_app_id").val(), false, function () {
            $("#appOperation").modal('hide');
            refresh();
        })
    });
    $("#app-enable").click(function () {
        chrome.management.setEnabled($("#modal_app_id").val(), true, function () {
            $("#appOperation").modal('hide');
            refresh();
        })
    });
    $("#uninstall").click(function () {
        ...
        $("#uninstall-real").click(function () {
            chrome.management.uninstall($("#modal_app_id").val(), {showConfirmDialog: false}, function () {
                $("#appOperation").modal('hide');
                refresh();
            })
        })
