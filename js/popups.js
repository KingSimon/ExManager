...
function showView() {
    ...
    for (var k = 0; k < groups.length; k++) {
        ...
        var groupHtml = '<div class="accordion-group">' +
            ...
            '<div class="span7">' +
            '<div class="row">' +
            '<div class="span5"><a class="accordion-toggle"  data-toggle="collapse" data-parent="#groupAccordion" href="#collapse' + k + '"> ' + groupName(group) + ' </a></div>' +
            '<div class="span2 banIcon">' +
            '<a class="btn btn-success btnEnableGroup" key="'+k+'"><i class="icon-play"></i></a>' +
            '<a class="btn btn-danger btnDisableGroup" key="'+k+'"><i class="icon-ban-circle"></i></a>' +
            '</div>' +
            '</div>' +
            '</div>' +
            '</div>' +
        ...
        $groupitem.find('.btnDisableGroup').click(function () {
            var key = $(this).attr('key');
            disableGroup(groups[key]);
        });
        $groupitem.find('.btnEnableGroup').click(function () {
            var key = $(this).attr('key');
            enableGroup(groups[key]);
        });
        $("#groupAccordion").append($groupitem);
    }
